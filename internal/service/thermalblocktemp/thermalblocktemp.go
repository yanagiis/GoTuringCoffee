...
func (t *Service) Run(ctx context.Context, nc *nats.EncodedConn, fin chan<- struct{}) (err error) {
	...
	nc.Subscribe("thermalblock.temperature", func(subj, reply string, req lib.Request) {
	...
	for {
		...
		case <-timer.C:
			...
			timer = time.NewTimer(t.ScanInterval)
		case <-ctx.Done():
			log.Info().Msg("stoping thermalblock temperature service")
			err = ctx.Err()
			defer func() { fin <- struct{}{} }()
			log.Info().Msg("stop thermalblock temperature service")
			return
		}
	}
}
...
func GetTemperature(ctx context.Context, nc *nats.EncodedConn) (resp lib.TempResponse, err error) {
	...
	err = nc.RequestWithContext(ctx, "thermalblock.temperature", &req, &resp)
	if err != nil {
		log.Error().Err(err).Msg("Get thermalblock temperature failed")
	}
	return
}
