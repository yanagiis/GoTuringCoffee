...
func (h *Service) Run(ctx context.Context, nc *nats.EncodedConn, fin chan<- struct{}) (err error) {
	nc.Subscribe("thermalblock.heater", func(subj, reply string, req lib.HeaterRequest) {
		if req.IsGet() {
...
func (h *Service) adjustTemperature(ctx context.Context, nc *nats.EncodedConn) error {
	resp, err := thermalblocktemp.GetTemperature(ctx, nc)
	if err != nil {
		h.pwm.PWM(0, 0)
		return err
	}
	if resp.IsFailure() {
		h.pwm.PWM(0, 0)
		return errors.New("Cannot get thermalblock temperature")
	}
...
func GetHeaterInfo(ctx context.Context, nc *nats.EncodedConn) (resp lib.HeaterResponse, err error) {
	...
	err = nc.RequestWithContext(ctx, "thermalblock.heater", &req, &resp)
	return
}
